...
// Edge-case fixtures use generated byte patterns and long tuples for clarity.
#![allow(
...
async fn test_offline_mode_sync_returns_offline() {
  ...
  with_timeout("test_offline_mode_sync_returns_offline", async {
    ...
    let (engine, _handle) = SyncEngine::start(SyncConfig::default(), backend.clone(), events.clone() as Arc<dyn Sink>);
...
async fn test_channel_closed_graceful() {
  ...
  with_timeout("test_channel_closed_graceful", async {
    ...
    let (engine, handle) = SyncEngine::start(SyncConfig::default(), backend.clone(), events.clone() as Arc<dyn Sink>);
