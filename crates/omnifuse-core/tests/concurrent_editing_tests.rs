...
async fn test_concurrent_write_same_file() {
  ...
  with_timeout("test_concurrent_write_same_file", async {
    ...
    let (fh_init, _) = vfs.create(&path, OpenFlags::read_write(), 0o644).await.expect("create");
    vfs
      .write(&path, fh_init, 0, &initial_data)
      .await
    ...
    let h1 = tokio::spawn(async move {
      let fh = vfs1.open(&path1, OpenFlags::read_write()).await.expect("open task1");
      // Write at offset 0
      vfs1.write(&path1, fh, 0, &data_a).await.expect("write task1");
      vfs1.flush(&path1, fh).await.expect("flush task1");
      vfs1.release(&path1, fh).await.expect("release task1");
    });

    let h2 = tokio::spawn(async move {
      let fh = vfs2.open(&path2, OpenFlags::read_write()).await.expect("open task2");
      // Write at offset 100 (non-overlapping with task1)
      vfs2.write(&path2, fh, 100, &data_b).await.expect("write task2");
    ...
    let fh_read = vfs.open(&path, OpenFlags::read_only()).await.expect("open for read");
    let content = vfs.read(&path, fh_read, 0, 200).await.expect("read all");
  ...
  with_timeout("test_concurrent_create_different_files", async {
    ...
    for i in 0..10u8 {
      ...
      handles.push(tokio::spawn(async move {
        ...
        vfs.write(&path, fh, 0, data.as_bytes()).await.expect("write");
    ...
    for i in 0..10u8 {
      ...
      let fh = vfs.open(&path, OpenFlags::read_only()).await.expect("open for verify");
      let data = vfs.read(&path, fh, 0, 1024).await.expect("read for verify");
      vfs.release(&path, fh).await.expect("release verify");

      let actual = String::from_utf8(data).expect("utf8");
      assert_eq!(actual, expected, "file_{i}.txt content mismatch: got {actual:?}");
    }
...
async fn test_concurrent_write_and_read() {
  ...
  with_timeout("test_concurrent_write_and_read", async {
    ...
    vfs.write(&path, fh, 0, initial).await.expect("write initial");
    vfs.flush(&path, fh).await.expect("flush");
    vfs.release(&path, fh).await.expect("release");
    ...
    let writer = tokio::spawn(async move {
      for round in 0..20u32 {
        let fh = vfs_w.open(&path_w, OpenFlags::read_write()).await.expect("open write");
        let data = format!("updated round {round:04}");
        vfs_w.write(&path_w, fh, 0, data.as_bytes()).await.expect("write");
...
async fn test_concurrent_rename_operations() {
  ...
  with_timeout("test_concurrent_rename_operations", async {
    ...
    for i in 0..10u32 {
      ...
      vfs.write(&path, fh, 0, data.as_bytes()).await.expect("write src");
    ...
    for i in 0..10u32 {
      ...
      assert!(src_result.is_err(), "src_{i}.txt should not exist after rename");
...
async fn test_write_during_sync_delay() {
  ...
  with_timeout("test_write_during_sync_delay", async {
    ...
    let fh2 = vfs.open(&path, OpenFlags::read_write()).await.expect("open v2");
    vfs.write(&path, fh2, 0, b"second version").await.expect("write v2");
...
async fn test_concurrent_flush_operations() {
  ...
  with_timeout("test_concurrent_flush_operations", async {
    ...
    for i in 0..10u32 {
      ...
      let fh = vfs.open(&path, OpenFlags::read_only()).await.expect("open verify");
      let data = vfs.read(&path, fh, 0, 1024).await.expect("read verify");
      vfs.release(&path, fh).await.expect("release verify");

      let actual = String::from_utf8(data).expect("utf8");
      assert_eq!(actual, expected, "flush_{i}.txt content mismatch: got {actual:?}");
    }
  ...
  with_timeout("test_concurrent_readdir_during_create", async {
    ...
    let creator = tokio::spawn(async move {
      for i in 0..total_files {
        ...
        let (fh, _) = vfs_c
          ...
          .await
          .expect("create");
        vfs_c.write(&path, fh, 0, b"content").await.expect("write");
    ...
    let reader = tokio::spawn(async move {
      ...
      for _ in 0..30u32 {
        ...
        let count = entries.iter().filter(|e| e.name.starts_with("readdir_")).count();
        if count > max_seen {
          max_seen = count;
        }
        tokio::task::yield_now().await;
    ...
    let final_count = entries.iter().filter(|e| e.name.starts_with("readdir_")).count();
    ...
    eprintln!("  [OK] readdir during create: max_seen={max_seen}, final={final_count}");
  })
  .await;
}
