...
use omnifuse_core::{
    ...
    test_utils::{MockBackend, TestEventHandler, with_timeout, TEST_TIMEOUT},
    vfs::OmniFuseVfs,
};
...
async fn test_e2e_write_triggers_sync() {
    ...
    with_timeout("test_e2e_write_triggers_sync", async {
        ...
        let (fh, _attr) = vfs
            ...
            .await
            .expect("create");
        vfs.write(path, fh, 0, b"hello world")
            .await
            .expect("write");
    ...
    with_timeout("test_e2e_multiple_files_batched", async {
        ...
        for name in &filenames {
            ...
            let (fh, _) = vfs
                ...
                .await
                .expect("create");
            vfs.write(path, fh, 0, format!("content of {name}").as_bytes())
                .await
                .expect("write");
            vfs.flush(path, fh).await.expect("flush");
...
async fn test_e2e_unlink_triggers_sync() {
    ...
    with_timeout("test_e2e_unlink_triggers_sync", async {
        ...
        let (fh, _) = vfs
            ...
            .await
            .expect("create");
        vfs.write(path, fh, 0, b"temporary content")
            .await
            .expect("write");
        vfs.flush(path, fh).await.expect("flush");
        vfs.release(path, fh).await.expect("release");
        ...
        assert!(
            !full_path.exists(),
            "ephemeral.txt should be deleted from disk"
        );
...
async fn test_e2e_rename_triggers_sync() {
    ...
    with_timeout("test_e2e_rename_triggers_sync", async {
        ...
        let (fh, _) = vfs
            ...
            .await
            .expect("create");
        vfs.write(old_path, fh, 0, b"rename me")
            .await
            .expect("write");
        ...
        assert!(
            renamed.iter().any(|(old, new)| {
                old.ends_with("old_name.txt") && new.ends_with("new_name.txt")
            }),
...
async fn test_e2e_sync_error_logged() {
    ...
    with_timeout("test_e2e_sync_error_logged", async {
        ...
        let (fh, _) = vfs
            ...
            .await
            .expect("create");
        vfs.write(path, fh, 0, b"this sync will fail")
            .await
            .expect("write");
        vfs.flush(path, fh).await.expect("flush");
        vfs.release(path, fh).await.expect("release");
        ...
        let has_error_msg = logs.iter().any(|(level, msg)| {
            *level == LogLevel::Error && msg.contains("remote storage unavailable")
        });
        assert!(
            has_error_msg,
            "error log should contain the backend error message"
        );
    })
    .await;
}
...
async fn test_e2e_sync_result_events() {
    ...
    with_timeout("test_e2e_sync_result_events", async {
        ...
        let (fh, _) = vfs
            ...
            .await
            .expect("create");
        vfs.write(path, fh, 0, b"quarterly report data")
            .await
            .expect("write");
        vfs.flush(path, fh).await.expect("flush");
        vfs.release(path, fh).await.expect("release");
...
async fn test_e2e_write_read_consistency() {
    ...
    with_timeout("test_e2e_write_read_consistency", async {
        ...
        let read_data = vfs
            .read(path, fh, 0, data.len() as u32)
            .await
            .expect("read");
        assert_eq!(
            read_data, data,
