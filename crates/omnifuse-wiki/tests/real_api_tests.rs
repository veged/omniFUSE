...
async fn test_smoke_crud_page() {
  ...
  // Read by id
  let fetched = client.get_page_by_idx(created.id).await.expect("get_page_by_idx");
  ...
  // Delete
  client.delete_page(created.id).await.expect("delete_page");
}
...
async fn test_get_page_tree() {
  ...
  let tree = client.get_page_tree(&slug, 100, 3).await.expect("get_page_tree");
  ...
  let has_children = tree.root.children.as_ref().is_some_and(|c| !c.is_empty());
...
async fn test_get_page_by_slug() {
  ...
  let page = client.get_page_by_slug(&slug).await.expect("get_page_by_slug");
